...
function App() {
  ...
  useEffect(() => {
    const fetchBookmarkLists = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/api/list/personal/${userId}?viewerId=${userId}`);
        if (response.ok) {
  ...
  const addBookmark = async (url) => {
    if (selectedListId && userId) {
      setLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/api/bookmark/add/${selectedListId}?userId=${userId}`, {
          method: 'POST',
          headers: {
        ...
        {userId ? (
          <>
            {bookmarkLists.length > 0 ? (
              ...
              <p>No bookmark lists available</p>
            )}
            {selectedListId && (
              <>
