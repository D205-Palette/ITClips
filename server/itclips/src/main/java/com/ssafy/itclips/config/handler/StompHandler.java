package com.ssafy.itclips.config.handler;
...
public class StompHandler implements ChannelInterceptor {
    ...
    public Message<?> preSend(Message<?> message, MessageChannel channel) {
        ...
        if (StompCommand.SUBSCRIBE == accessor.getCommand()) { // 채팅룸 구독요청
            ...
            chatService.sendChatMessage(ChatMessage.builder().messageType(ChatMessage.MessageType.ENTER).roomId(roomId).sender(name).build());
            ...
            String name = Optional.ofNullable((Principal) message.getHeaders().get("simpUser")).map(Principal::getName).orElse("UnknownUser");
            chatService.sendChatMessage(ChatMessage.builder().messageType(ChatMessage.MessageType.QUIT).roomId(roomId).sender(name).build());
